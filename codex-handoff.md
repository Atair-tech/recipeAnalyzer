...
Latest updater verification follow-up on 2026-05-27: local versions remain aligned at `0.1.29` in `package.json`, `src-tauri/tauri.conf.json`, and local `src-tauri\target\release\bundle\nsis\latest.json`. Local NSIS assets exist for `Recipe Analyzer_0.1.29_x64-setup.exe` and `.sig`. Anonymous network checks from this machine returned HTTP 200 for `https://github.com/Atair-tech/recipeAnalyzer/releases/latest/download/latest.json` and HTTP 200 for HEAD on `https://github.com/Atair-tech/recipeAnalyzer/releases/download/v0.1.29/Recipe.Analyzer_0.1.29_x64-setup.exe`, with content length `47073934`. The installed updater test still needs to be run from a manually installed `0.1.29` build; if it fails, collect the new System Settings diagnostics, especially the preflight message, installer URL, proxy value, timeout, and any `UPDATE_INSTALL_FAILED` detail.

Latest editor user-view follow-up on 2026-05-27: added a third `用户阅览` tab to the web editor alongside `浏览数据` and `执行 SQL`. Backend now exposes `GET /api/recipes/editor/user-views` and `POST /api/recipes/editor/user-view-rows`. The first user-facing views are `菜谱阅览`, `标准食材索引`, and `自动标签索引`; they are read-only, paginated, column-filterable, and use user-facing column labels instead of raw table schemas. Frontend `RecipeEditor.jsx` adds a view picker, refresh, clear-filter, pagination, sticky filter row, and resizable read-only columns for this tab. Verified `py_compile` for `recipe.py`, `routes/recipes.py`, and `recipe_service.py`; targeted service calls returned all three views and paginated rows; `npm --prefix frontend run build` passed. No desktop package was rebuilt.

Immediate fix after user-view addition: the already-running backend still lacked the new `/api/recipes/editor/user-views` endpoint, so the editor initialization saw a 404 and left all tabs empty. `RecipeEditor.jsx` now loads the original schema/table metadata first and treats user-view API failure as non-fatal, preserving `浏览数据` and `执行 SQL` if the backend is stale. Restarted the local backend on port 8000; `GET /api/recipes/editor/user-views` now returns 200 and `POST /api/recipes/editor/user-view-rows` for `recipes` returned `total=1349`, `items=3`, `columns=20`. Rebuilt frontend with `npm --prefix frontend run build`.

Latest user-view Excel-style filter follow-up on 2026-05-27: replaced the `用户阅览` tab's second-row text filters with Excel-like header dropdown filters. Each user-view field header now shows only the field name plus a small down-arrow; field data type labels such as `TEXT`/`INTEGER` are no longer shown in either `用户阅览` or the raw `浏览数据` headers. Added backend `POST /api/recipes/editor/user-view-filter-values`, plus array-value filtering support in the shared table filter builder. The frontend dropdown fetches distinct values for the selected field, supports search, visible-value select all, checkboxes, clear, confirm, and cancel. Verified Python compile, service-level Unicode column filtering, frontend build, restarted the local backend, and HTTP-tested `user-view-filter-values` on the `ID` field.

Small user-view filter UX fix: clicking the same header down-arrow again now closes the open filter dropdown instead of reopening it. Verified with `npm --prefix frontend run build`.

Latest user-view sort follow-up on 2026-05-29: made the `用户阅览` filter menu's `升序` and `降序` commands functional instead of disabled. `UserViewRowsPayload` now accepts `sort_column` and `sort_direction`; `list_user_view_editor_rows()` validates the requested column, uses numeric ordering for numeric view columns, and returns the active sort metadata. Frontend `RecipeEditor.jsx` stores user-view sort state, sends it with `fetchRecipeEditorUserViewRows()`, closes the dropdown after sort selection, and shows an up/down indicator in the sorted header. Verified backend `py_compile`, service-level sort on `标准食材索引.关联菜谱数`, `npm --prefix frontend run build`, restarted local backend, and HTTP-tested sorting `ingredients` by `ID desc`.

Latest user-view recipe-column follow-up on 2026-05-29: read `docs/编辑模式用户阅览.xlsx` and changed the `用户阅览` tab's `菜谱阅览` view to match its listed fields. The view now omits fields not mentioned in that workbook, including raw `ID`, `分类`, `系统备注`, and old manual `标签`; displayed fields are `序号` from the frontend row-number column plus backend columns `菜名`, `主题`, `分组`, `大地域`, `细分地域`, `食材`, `调料`, `做法与要点`, `来源/修订备注`, `最后记录日期`, `BMD`, `CC`, `自动标签（参考）`, `标准食材（参考）`, `记录类型`, and `待办状态`. Frontend default user-view column widths were tightened by field name while keeping the existing drag-to-resize behavior. Verified service-level schema/row output, backend `py_compile`, `npm --prefix frontend run build`, restarted local backend, and HTTP-tested the updated `recipes` user view columns.

Latest user-view compact-width follow-up on 2026-05-29: further reduced default `用户阅览` column widths so the `菜谱阅览` fields fit roughly within one screen. Lowered the resize minimum from 56px to 34px, narrowed the row-number column to 48px, and tightened the filter-arrow header spacing so users can manually drag columns narrower. User-view cells now remain single-line with ellipsis; clicking a cell opens a fixed-position popover showing the full content with line wrapping and scrolling. Verified `npm --prefix frontend run build`.

Immediate user-view width fix: content was still stretching columns because the user-view table inherited the generic table `min-width: 100%`/intrinsic table sizing behavior. `RecipeEditor.jsx` now computes the user-view table width as the row-number width plus current column widths and applies that exact width/minWidth to the table; `.user-view-table` also overrides the generic min-width. This makes default widths and manual drag-resize take effect. Verified `npm --prefix frontend run build`.

Small user-view polish follow-up: user-view default widths now enforce a label-width floor so every column can fully show its field name, and the full-cell popover closes automatically when the user clicks outside the popover or outside another user-view cell. Verified `npm --prefix frontend run build`.

Immediate user-view filter semantics fix: Excel-style multi-select filters in `用户阅览` were using exact whole-cell `IN (...)` matching, so selecting values such as `牛肉` in the `食材` column did not filter rows whose ingredient text contained `牛肉` among other text. `list_user_view_editor_rows()` now calls `_build_table_editor_filters(..., list_match_mode="contains")`, while raw table browsing keeps exact list matching. Service-level test showed `菜谱阅览` filtering `食材=[牛肉]` reduced total rows from 1349 to 23; backend `py_compile` passed; local backend was restarted and HTTP-tested with the same filter returning `TOTAL=23`.

Second user-view filter fix: after typing a search term in the Excel-style dropdown, all visible search results were selected by default, and `applyUserFilterMenu()` treated that as "all values selected" and deleted the filter. It now only clears the filter on all-selected when the search box is empty; searched selections are preserved as active filters. Verified `npm --prefix frontend run build`.

Small editor tab order follow-up: `RecipeEditor.jsx` now defaults to the `用户阅览` tab when opening `#editor`, and the top tab order is `用户阅览`, `浏览数据`, `执行 SQL`. Verified `npm --prefix frontend run build`.

Latest standard-ingredient user-view follow-up on 2026-05-29: updated `用户阅览 -> 标准食材索引` by removing the `ID` column from the backend view/schema and increasing frontend default widths for `可见` and `关联菜谱数` so their headers display fully. Verified service-level schema/row keys (`标准食材`, `可见`, `关联菜谱数`, `别名`), backend `py_compile`, `npm --prefix frontend run build`, and restarted the local backend.

Latest automatic-tag user-view follow-up on 2026-05-29: user requested changes under `标准食材索引` but named columns (`说明`, `排序`) that exist in `自动标签索引`; applied the change to `自动标签索引` while preserving the already-updated `标准食材索引`. Removed `ID` and `排序` from the automatic-tag backend view/schema, leaving `标签`, `说明`, `可见`, and `关联菜谱数`; increased the frontend default width for `说明` to 260 and kept `可见`/`关联菜谱数` wide enough for their headers. Verified service-level schemas for `ingredients` and `managed_tags`, backend `py_compile`, `npm --prefix frontend run build`, and restarted the local backend.

Latest desktop package follow-up on 2026-05-29: bumped app/package/Tauri/Rust versions from `0.1.29` to `0.1.30` for the editor user-view work and built a new NSIS installer. Pre-checks passed: backend `py_compile` for recipe schemas/routes/service and `npm --prefix frontend run build`. Full `npm run desktop:build` built the sidecar and release app but failed during MSI WiX `light.exe`; then `tauri build --bundles nsis` initially failed because `data\recipes.xlsx` was open in Excel, then succeeded after the workbook was closed. Latest NSIS installer: `src-tauri\target\release\bundle\nsis\Recipe Analyzer_0.1.30_x64-setup.exe`; signature: `Recipe Analyzer_0.1.30_x64-setup.exe.sig`; SHA256 `BC689791BC9C81219A100EFA090507E5B0F4F4033F0983DB091D2AAD41DD74D7`. Regenerated updater metadata at `src-tauri\target\release\bundle\nsis\latest.json` for release tag `v0.1.30`. Build still emitted the known non-fatal bundler-type patch file-lock warning for the updater plugin before NSIS succeeded.
